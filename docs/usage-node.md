...
Package: [`@graphnight/sdk`](https://www.npmjs.com/package/@graphnight/sdk) **1.0.0** — TypeScript client for GraphNight GraphQL API.

The Node.js client talks to a remote GraphNight GraphQL server. For local YAML storage, use the [CLI](usage-cli.md) or [Python SDK](usage-python.md).
...
```typescript
const client = createClient({
  url: 'http://localhost:8080/graphql',     // GraphQL endpoint (required for server mode)
  storagePath: './graphnight_data',          // Local YAML storage (for local mode)
...
```typescript
const updated = await client.updateModel('orders', {
...
```typescript
import { GraphNightClient } from '@graphnight/sdk';
